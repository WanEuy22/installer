...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/WanEuy22/AccesIP/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/WanEuy22/AccesIP/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/WanEuy22/AccesIP/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/WanEuy22/AccesIP/main/ip | grep $MYIP | awk '{print $3}')
fi
clear
MYIP=$(curl -sS ipv4.icanhazip.com)
#########################
...
if [[ $ssh_status == "running" ]]; then 
   status_ssh=" ${GREEN}Service Running ${NC}"
else
   status_ssh="${RED}  Service Error ${NC}"
fi

# STATUS SERVICE OPENVPN
if [[ $oovpn == "active" ]]; then
  status_openvpn=" ${GREEN}Service Running ${NC}"
else
  status_openvpn="${RED}  Service Error ${NC}"
fi
...
if [[ $squid_service == "running" ]]; then 
   status_squid=" ${GREEN}Service Running ${NC}"
else
   status_squid="${RED}  Service Error ${NC}"
fi

# STATUS SERVICE  VNSTAT 
if [[ $vnstat_service == "running" ]]; then 
   status_vnstat=" ${GREEN}Service Running ${NC}"
else
   status_vnstat="${RED}  Service Error ${NC}"
fi

# STATUS SERVICE  CRONS 
if [[ $cron_service == "running" ]]; then 
   status_cron=" ${GREEN}Service Running ${NC}"
else
   status_cron="${RED}  Service Error ${NC}"
fi

# STATUS SERVICE  FAIL2BAN 
if [[ $fail2ban_service == "running" ]]; then 
   status_fail2ban=" ${GREEN}Service Running ${NC}"
else
   status_fail2ban="${RED}  Service Error ${NC}"
fi

# STATUS SERVICE  TLS 
if [[ $tls_v2ray_status == "running" ]]; then 
   status_tls_v2ray=" ${GREEN}Service Running${NC}"
else
   status_tls_v2ray="${RED}  Service Error${NC}"
fi

# STATUS SERVICE NON TLS V2RAY
if [[ $nontls_v2ray_status == "running" ]]; then 
   status_nontls_v2ray=" ${GREEN}Service Running ${NC}"
else
   status_nontls_v2ray="${RED}  Service Error ${NC}"
fi
...
if [[ $trojan_server == "running" ]]; then 
   status_virus_trojan=" ${GREEN}Service Running ${NC}"
else
   status_virus_trojan="${RED}  Service Error ${NC}"
fi
...
if [[ $dropbear_status == "running" ]]; then 
   status_beruangjatuh=" ${GREEN}Service Running${NC}"
else
   status_beruangjatuh="${RED}  Service Error ${NC}"
fi
...
echo -e "${GREEN}»${NC} Server IP          = "
echo -e "${GREEN}»${NC} Total Ram          = "
echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m${NC}"
echo -e "\E[41;1;39m               ⇱ Status Layanan ⇲               \E[0m"
echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m${NC}"
echo -e "${GREEN}»${NC} SSH Websocket HTTP      = $status_ssh"
echo -e "${GREEN}»${NC} SSH Websocket SSL       = $status_ssh"
echo -e "${GREEN}»${NC} OpenVPN                 = $status_openvpn"
echo -e "${GREEN}»${NC} Dropbear                = $status_beruangjatuh"
echo -e "${GREEN}»${NC} Squid                   = $status_squid"
echo -e "${GREEN}»${NC} Fail2Ban                = $status_fail2ban"
echo -e "${GREEN}»${NC} Crons                   = $status_cron"
echo -e "${GREEN}»${NC} Vnstat                  = $status_vnstat"
echo -e "${GREEN}»${NC} XRAY Vmess TLS          = $status_tls_v2ray"
echo -e "${GREEN}»${NC} XRAY Vmess None TLS     = $status_nontls_v2ray"
echo -e "${GREEN}»${NC} Shadowsocks-R           = $status_ssh"
echo -e "${GREEN}»${NC} Shadowsocks-OBFS HTTPS  = $status_ssh"
echo -e "${GREEN}»${NC} Shadowsocks-OBFS HTTP   = $status_ssh"
echo -e "${GREEN}»${NC} XRAY Trojan             = $status_virus_trojan"
echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m${NC}"
echo ""
